    ...
    for i, json_file in enumerate(json_files):
        try:
            ...
            for msg in messages:
                ...
                # 使用 observation_type 而不是直接设置 node_type
                observation_type = 'USER_INPUT' if msg['role'] == 'user' else 'ASSISTANT_RESPONSE'
                ...
                """, (
                    node_id, WORKSPACE_ID, 1, 'UserInput' if msg['role'] == 'user' else 'AssistantResponse',
                    content_text, summary,
                    'active', 'chatgpt', 'system',
                    json.dumps([evidence_id]), '[]', observation_type,
                    json.dumps({}),
                    now, now
                ))
            ...
            if (i + 1) % 50 == 0:
                ...
                print(f"  [{i+1}/{len(json_files)}] {rate:.1f} 对话/秒, 预计剩余 {eta:.0f} 秒, 消息: {total_messages}")
