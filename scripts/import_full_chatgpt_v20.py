    ...
    for i, json_file in enumerate(json_files):
        try:
            ...
            for entity_name in list(entities_found)[:20]:
                # 检查是否已存在同名area
                if entity_name in existing_areas:
                    area_id = existing_areas[entity_name]
                else:
                    ...
                    try:
                        ...
                        existing_areas[entity_name] = area_id
                    except psycopg2.errors.UniqueViolation:
                        # 如果已存在，查询获取已有的area_id
                        cur.execute("SELECT id FROM areas WHERE workspace_id = %s AND name = %s", (WORKSPACE_ID, entity_name))
                        row = cur.fetchone()
                        if row:
                            area_id = row[0]
                            existing_areas[entity_name] = area_id
